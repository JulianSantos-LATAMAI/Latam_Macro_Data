# 🌎 LATAM - U.S. - Spain: Business Intelligence



*Connecting the dots with Santiago, Madrid, Miami and everywhere in between.*


### 📌 About This Repository
This repository serves as a living portfolio of **data-driven insights** regarding the economic exchange between **Latin America, Spain, and the United States**.

As an International Business student and Analyst, my goal is to strip away the noise and use data to validate real investment theses. This space will house various projects, charts, and reports that track:
* **Foreign Direct Investment (FDI)** flows.
* **Market Entry Opportunities** for the "Air Bridge" (LATAM → US/Europe).
* **Macro-Economic Trends** that impact cross-border business.

---

### 🇨🇱 Current Project: The "Chile Investment Paradox"


### 🔎 Data Sources
All insights in this repository are derived from official, open-source economic databases to ensure integrity for business decision-making.
* **Primary Source:** World Bank Open Data
* **Focus Areas:** Net Inflows, GDP Growth Rates, Trade Balances.

---

### 👤 Author
**Julián Santos**
**Role:** International Business Student @ Northeastern University 
* **Experience:** International Business Analyst @ Spain-U.S. Chamber of Commerce 
  **Leadership:** Founder & Director, Latam Strategy Group
\

*Connecting the dots between Santiago, Madrid, and Miami.*

[LinkedIn](www.linkedin.com/in/juliansvurl) 
[Email] (santos.jul@northeastern.edu)
